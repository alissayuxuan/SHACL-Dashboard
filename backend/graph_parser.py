...
def filterNode(graph, node):
    ...
    fn_most_frequent_violationtype = extract_sparql_result(graph.query(fn_queryMostFrequentViolationType))
    total_violations = extract_sparql_result(graph.query(queryGesamtzahlViolations))
...
def nichtBEnutzt(graph, node):
    print("anfangslength: ", len(graph))
    print("nodeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee: " + str(node))
    queryFilterNode = f"""DESCRIBE <{node}>"""
    
    res = graph.query(queryFilterNode)
    ...
    filtered_graph.parse(data=res.serialize(format="xml"), format="xml")
...
def check_node_exists(graph, node):
    print(len(graph))
    # Suche nach Tripeln, bei denen der Knoten als Subjekt, Prädikat oder Objekt vorkommt
    print("Suche nach Tripeln mit dem Knoten:")
    triples_with_node = list(graph.triples((None, None, node))) + \
                        list(graph.triples((node, None, None))) + \
                        list(graph.triples((None, node, None)))
    
    print(f"Anzahl gefundener Tripel: {len(triples_with_node)}")
    
    if triples_with_node:
        for s, p, o in triples_with_node:
            print(f"Gefunden: {s} | {p} | {o}")
    
    return len(triples_with_node) > 0
...
#node = URIRef("http://www.Department15.University4.edu/FullProfessor1")
#check_node_exists(g, node)




#print(f"Filtered Graph has {len(g)} triples.")


a = filterNode(g, URIRef("result"))

print(f"Filtered Graph has {len(a)} triples.")
...
#a = extract_sparql_result(g.query(queryMostViolatingNode))
#b = prefixEntfernenEinzeln(str(a), g)
#print(a)
#print(b)


#res = analyze_graph(g)
#print(res["most_violating_node"])
#rint(res["focusNode_violations"])

#result = filterNode(g, URIRef("http://www.Department5.University0.edu/FullProfessor7"))


#re = resultSeverityDistribution(g)
